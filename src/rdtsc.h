...
static __inline__ uint64_t curtick() {
    ...
    // TODO specify for ARM. Will rely on time instead of cycles for now
    tick = 0;
#endif
	return tick;
}
...
static __inline__ void resumeTimer(uint64_t* t) {
    *t = curtick() - *t;
}
...
static __inline__ void resetTimer(uint64_t* t) {
    *t = 0;
}
