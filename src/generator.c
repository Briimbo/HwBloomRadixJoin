...
// clang-format off
#define RAND_RANGE(O, N) ((O) + ((double)rand() / ((double)RAND_MAX + 1) * ((N) - (O))))
...
struct create_arg_t {
    relation_t          rel;
    int64_t             firstkey;
    int64_t             firstkey_above;
    int64_t             threshold;
    uint64_t            numtuples_above;
    uint64_t            ridstart;
    relation_t *        fullrel;
    volatile void *     locks;
 ...
 * To simulate a WHERE predicate, the number of keys
 * above a threshold that will not find a join partner
...
random_unique_gen_thread(void * args)
{
    ...
    int64_t        firstkey_above  = arg->firstkey_above;
    int64_t        threshold       = arg->threshold;
    uint64_t       numtuples_below = rel->num_tuples - arg->numtuples_above;
    ...
    for (i = 0; i < numtuples_below; i++) {
        rel->tuples[i].key     = firstkey;
        rel->tuples[i].payload = ridstart + i;

        if (firstkey == threshold) firstkey = 0;

        firstkey++;
    }

    for (; i < rel->num_tuples; i++) {
        rel->tuples[i].key     = firstkey_above;
        rel->tuples[i].payload = ridstart + i;

        if (firstkey_above == INT_MAX) firstkey_above = threshold;
 ...
 * relation must have been allocated
 * tuple keys are between minid(inclusive) and maxid(inclusive)
 */
void
random_gen(relation_t * rel, const int64_t minid, const int64_t maxid)
{
    uint64_t i;

    for (i = 0; i < rel->num_tuples; i++) {
        rel->tuples[i].key     = RAND_RANGE(minid, maxid);
        rel->tuples[i].payload = i;
    }
}
...
parallel_create_relation(relation_t * relation, uint64_t num_tuples,
                         uint32_t nthreads, uint64_t maxid, uint64_t threshold,
                         double selectivity)
{
    int      rv;
    uint32_t i;
    uint64_t offset       = 0;
    uint64_t offset_above = 0;
    ...
    uint64_t     ntuples_above;
    uint64_t     ntuples_above_perthr;
    uint64_t     ntuples_above_lastthr;
    ...
    ntuples_above  = num_tuples * (1 - selectivity);

    if (npages_perthr == 0) ntuples_perthr = num_tuples / nthreads;

    ntuples_above_perthr = ntuples_perthr * (1 - selectivity);
    ntuples_lastthr      = num_tuples - ntuples_perthr * (nthreads - 1);
    ntuples_above_lastthr = ntuples_above - (nthreads - 1) * ntuples_above_perthr;
    ...
    for (i = 0; i < nthreads; i++) {
        ...
        args[i].firstkey       = (offset + 1) % threshold;
        args[i].firstkey_above = threshold
                                 + (offset_above + 1)
                                       % (MAX(1, maxid - threshold));
        args[i].threshold       = threshold;
        args[i].numtuples_above = (i == nthreads - 1) ? ntuples_above_lastthr
                                                      : ntuples_above_perthr;
        args[i].ridstart        = offset + offset_above;
        args[i].rel.tuples      = relation->tuples + offset + offset_above;
        ...
        offset += ntuples_perthr - ntuples_above_perthr;
        offset_above += ntuples_above_perthr;
...
create_relation_fk_from_pk(relation_t * fkrel, relation_t * pkrel,
                           int64_t num_tuples, uint64_t threshold,
                           double selectivity)
{
    int      rv, i, iters;
    int64_t  remainder;
    uint64_t ntuples_above;
    uint64_t ntuples_below;
    ...
    ntuples_above = num_tuples * (1 - selectivity);
    ntuples_below = num_tuples - ntuples_above;

    /* generate new (unmatched) tuples first */
    fkrel->num_tuples = ntuples_above;
    fkrel->tuples     = fkrel->tuples + ntuples_below;
    random_gen(fkrel, threshold + 1, INT_MAX);

    fkrel->num_tuples = ntuples_below;
    fkrel->tuples     = fkrel->tuples - ntuples_below;

    /* alternative generation method */
    iters = ntuples_below / pkrel->num_tuples;
    for (i = 0; i < iters; i++) {
    ...
    remainder = ntuples_below % pkrel->num_tuples;
    if (remainder > 0) {
        memcpy(fkrel->tuples + i * pkrel->num_tuples, pkrel->tuples,
...
int
create_relation_nonunique_from_pk(relation_t * relation, relation_t * pkrel,
                                  int64_t num_tuples, const int64_t threshold,
                                  double selectivity)
{
    uint64_t ntuples_above;
    int      i;
    ...
    relation->num_tuples = ntuples_above;
    random_gen(relation, threshold + 1, INT_MAX);
    relation->num_tuples = num_tuples;

    for (i = ntuples_above; i < num_tuples; i++) {
        j = RAND_RANGE(0, pkrel->num_tuples);

        relation->tuples[i].key     = pkrel->tuples[j].key;
