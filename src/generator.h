 ...
 */
int
create_relation_nonunique(relation_t * reln, int64_t ntuples,
                          const int64_t maxid);

/**
 * Create relation with non-unique keys uniformly distributed between [0,
 * maxid], where <selectivity>% of keys are located in the upper range
 * [threshold + 1, INT_MAX] and 1 - <selectivity>% of keys are taken from the
 * primary key relation, which should only contain keys between [0, threshold]
 */
int
 ...
 * foreign-key relation size. selectivity * ntuples keys are taken from
 * the primary-key relation, the remaining keys will be in the range
 * (threshold, INT_MAX]
 *
...
create_relation_fk_from_pk(relation_t * fkrel, relation_t * pkrel,
                           int64_t ntuples, uint64_t threshold,
                           double selectivity);
 ...
 */
int
create_relation_zipf(relation_t * reln, int64_t ntuples, const int64_t maxid,
 ...
 */
int
parallel_create_relation(relation_t * reln, uint64_t ntuples, uint32_t nthreads,
                         uint64_t maxid, uint64_t threshold, double selectivity);
