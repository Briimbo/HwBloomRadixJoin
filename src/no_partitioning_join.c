...
struct arg_t {
    ...
    struct timeval start, probe_start, end;
#endif
} ;
...
print_timing(uint64_t total, uint64_t build, uint64_t part,
            ...
            struct timeval * start, double probe_usec, struct timeval * end)
{
...
NPO_st(relation_t *relR, relation_t *relS, int nthreads)
{
    ...
    double probe_usec = (end.tv_sec * 1000000L + end.tv_usec
             - (probe_start.tv_sec * 1000000L + probe_start.tv_usec));
    print_timing(timer1, timer2, timer3, relS->num_tuples, result, &start, probe_usec, &end);
#endif
...
npo_thread(void * param)
{
    ...
    if(args->tid == 0){
        ...
        gettimeofday(&args->probe_start, NULL);
    }
#endif
...
#ifndef NO_TIMING
    /* now print the timing results: */
    double probe_usec = (args[0].end.tv_sec * 1000000L + args[0].end.tv_usec
                         - (args[0].probe_start.tv_sec * 1000000L
                            + args[0].probe_start.tv_usec));
    print_timing(args[0].timer1, args[0].timer2, args[0].timer3, relS->num_tuples,
                 result, &args[0].start, probe_usec, &args[0].end);
#endif
